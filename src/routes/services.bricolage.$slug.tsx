...
export const Route = createFileRoute("/services/bricolage/$slug")({
  loader: ({ params }) => {
    const f = getBySlug(params.slug);
    if (!f || f.category !== "bricolage") throw notFound();
    return f;
  },
  head: ({ loaderData }) => ({
    meta: loaderData ? [
  ...
  component: () => {
    ...
    return <FranchiseSingle franchise={f} backTo="/services/bricolage" backLabel="Bricolage" />;
  },
});
