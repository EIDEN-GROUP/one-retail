...
export const Route = createFileRoute("/services/restauration/$slug")({
  loader: ({ params }) => {
    const f = getBySlug(params.slug);
    if (!f || f.category !== "restauration") throw notFound();
    return f;
  },
  head: ({ loaderData }) => ({
    meta: loaderData ? [
  ...
  component: () => {
    ...
    return <FranchiseSingle franchise={f} backTo="/services/restauration" backLabel="Restauration & Cafés" />;
  },
});
